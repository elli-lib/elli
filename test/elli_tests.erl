...
teardown(Pids) -> [elli:stop(P) || P <- Pids].
...
get_pipeline() ->
  ...
  ?assertEqual(binary:copy(ExpectedResponse, 2), Res).
...
start_slow_client(Port, Url) ->
  case gen_tcp:connect("127.0.0.1", Port, [{active, false}, binary]) of
    {ok, Socket} ->
      ...
      Socket;
    {error, Reason} -> throw({slow_client_error, Reason})
  end.
...
invalid_callback_test() ->
  case catch elli:start_link([{callback, elli}]) of
    E -> ?assertEqual(invalid_callback, E)
  end.
