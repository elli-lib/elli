...
-export([get_acceptors/1, get_open_reqs/1, get_open_reqs/2, set_callback/3]).
...
remove_acceptor(#state{acceptors = Acceptors} = State, Pid) ->
  ets:delete(Acceptors, Pid),
  dec_open_reqs(State).

-spec start_add_acceptor(State0 :: state()) -> State1 :: state().
start_add_acceptor(#state{options = Opts, callback = Callback} = State) ->
  Pid = elli_http:start_link(self(), State#state.socket, Opts, Callback),
