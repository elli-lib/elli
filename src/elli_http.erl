...
handle_request(S, PrevB, Opts, {Mod, Args} = Callback) ->
  {Method, RawPath, V, B0} =
    get_request(S, PrevB, Opts, Callback), t(request_start),
  {RequestHeaders, B1} =
    get_headers(S, V, B0, Opts, Callback), t(headers_end),
  Req = mk_req(Method, RawPath, RequestHeaders, <<>>, V, S, Callback),
  case init(Req) of
    {ok, standard} ->
      {RequestBody, B2} =
        get_body(S, RequestHeaders, B1, Opts, Callback), t(body_end),
  ...
  case proplists:get_value(<<"Content-Length">>, Headers, undefined) of
    ...
    ContentLengthBin ->
      ...
      case ContentLength - byte_size(Buffer) of
        0            -> {Buffer, <<>>};
        N when N > 0 -> do_get_body(Socket, Buffer, Opts, N, Callback);
        _ ->
...
connection_token(#req{version = {0, 9}}) -> <<"close">>.
...
connection(Req, UserHeaders) ->
  case proplists:get_value(<<"Connection">>, UserHeaders) of
    undefined -> {<<"Connection">>, connection_token(Req)};
    _         -> []
  end.
...
parse_path(_) -> {error, unsupported_uri}.
...
t(Key) -> put({time, Key}, os:timestamp()).

get_timings() -> lists:filtermap(fun get_timings/1, get()).

get_timings({{time, accepted}, Value}) -> {true, {accepted, Value}};
